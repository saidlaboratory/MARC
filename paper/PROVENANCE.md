...
| R20 | v0.3 repair ranker, linear `shared` held out | **0.565 [0.516,0.613]** vs candidate-only 0.343 / random 0.283 (N=400); all-pattern paired p=5.4e-26 | `python3 scripts/run_repair_ranker.py --train-data aux_required --exclude-family shared --n-train 1600 --n-val 400 --n-test 1200 --epochs 160 --batch-size 32 --D 128 --L 4 --lr 0.0007 --out results/p_repair/random_support_holdout_shared.json --ckpt checkpoints/repair_random_support_holdout.pt`, then checkpoint-only paired replay | data seed 20260722; disjoint +500000/+900000 ranges; Data Version 6 | working tree after 71f083c |
| R21 | v0.3 repair ranker, balanced nonlinear | **0.889 [0.852,0.917]** vs candidate-only 0.422 / random 0.253 (N=360); paired p=4.1e-46 | `python3 scripts/run_repair_ranker.py --train-data nonlinear --n-train 320 --n-val 120 --n-test 360 --epochs 120 --batch-size 16 --D 96 --L 3 --lr 0.0008 --out results/p_repair/nonlinear_balanced_full.json --ckpt checkpoints/repair_nonlinear_balanced_full.pt`, then checkpoint-only paired replay | data seed 20260722; Data Version 6 balanced offsets | working tree after 71f083c |
| R22 | v0.3 repair ranker, nonlinear multi-seed robustness | **0.889 / 0.875 / 0.881**, mean 0.881, SD 0.006; candidate-only mean 0.417, random 0.253 (same N=360) | `python3 scripts/run_repair_multiseed.py --data nonlinear --n-train 320 --n-val 120 --n-test 360 --epochs 100 --batch-size 16 --D 96 --L 3 --lr 0.0008 --out results/p_repair/nonlinear_multiseed.json` | data seed 20260722; train seeds 11/29/47 | working tree after 71f083c |
| R23 | v0.3 end-to-end repair + solver | linear **0.540** vs 0.333/0.253, oracle=enum=1.000 (N=300); nonlinear **0.883** vs 0.433/0.250, oracle=enum=0.950 (N=60) | `scripts/run_repair_ranker.py --eval-only --solve-e2e ...`; exact solver linear, matched refine (k=4, polish=4000) nonlinear; common restart seeds | exact commands/config in `linear_e2e.json`, `nonlinear_e2e_matched.json` | working tree after 71f083c |
| R24 | v0.3 cross-budget K scaling | K=4/8/16 full 0.540/0.347/0.247; random 0.253/0.140/0.020; speedup 1.21x/2.35x/3.91x | checkpoint-only `--solve-e2e --K {4,8,16}`; `linear_e2e.json`, `linear_K8_e2e.json`, `linear_K16_e2e.json` | shared data seed protocol; N=300/150/150 | working tree after 71f083c |
