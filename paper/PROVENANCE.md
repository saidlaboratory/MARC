...
| R15b | **Dimension-scaling seed robustness (3 seeds)** | crossover ROBUST at n=3/4 (learned 0.867±0.113 / 0.933±0.029 vs random 0.067/0.017, every seed); n=1 learned 0.867±0.138 loses to random 1.0; **n=6 learned 0.742±0.426 UNSTABLE** (range 0.25–1.0) — single-seed scaling.json 0.250 is the low outlier | `PYTHONPATH=. python3 scripts/run_dimension_scaling.py --seeds 3 --out results/p_scaling/scaling_3seed.json` | 200 epochs, 40 test/n/seed, self-trained x0; separate --out leaves cited scaling.json untouched; `results/p_scaling/scaling_3seed.json` | quang/geo-repair |
...
| R29b | **Oracle-marginal control, geometry point chains** (R29 for the 2nd coupled family) | oracle marginals tie random at every k (0.700/0.175/0.025/0.000 vs random 0.625/0.175/0.025/0.000; 0/4 significant wins, p=0.24 at k=1); random arm reproduces R25 digit-for-digit; marginals causally insufficient under coupling on BOTH families | `PYTHONPATH=.:scripts python3 scripts/run_oracle_marginal_pointchain.py --trials 40 --K 8` | pool seed0=100+k (disjoint from test 90000+k); best-of-8 geometry polish + Checker; `results/p_geometry/oracle_marginal_pointchain.json` | quang/geo-repair |
| R30 | **Real-systems construction repair — the external anchor (POSITIVE)** | same vocabulary as R28, hardened variants of the named real systems, two-stream selection: `trilat_far` ceiling **1.000** vs restart+4V 0.379 (N_fail=174, McNemar p=3.1e-33); `conic_ghost` ceiling **1.000** vs 0.107 (N_fail=56, p=8.9e-16); `ik3r_random`/`circles_far` = documented negatives (LM never fails). Structural-branch traps, not reachability flukes — the failure mode decides. Portfolio/enum repair (constant branch within a class), not per-instance selection | `PYTHONPATH=. python3 scripts/pilot_real_repair.py --n 200 --out results/p_real_repair/real_repair.json` | reference LM k=4, numeric acceptance max\|r\|<1e-6 on ORIGINAL factors; 2-stream failure selection; CRN grading; `results/p_real_repair/real_repair.json` | quang/geo-repair |
| R31 | **3D DMDGP construction repair — the 2D trap holds in native 3D** (negative) | k=10 ceiling 0.48 vs restart+32 0.70 (N=27, McNemar p=0.996); k=12 ceiling 0.60 vs 0.81 (N=42, p=0.998) — restarts beat constructions at both k. Geometry validated by tests (planted config exact root; pins split at true branch; Cayley-Menger checks). Failure mode decides: synthetic pruned chains (R28/R31)=fluke, real systems (R30)=structural trap | `PYTHONPATH=. python3 scripts/pilot_geo_repair3d.py --n 80 --ks 10,12` | two-stream failure selection; CRN grading seed+3*SALT; GEO_REPAIR3D_VERSION 1; `results/p_geo_repair/pilot3d.json` | quang/geo-repair |
