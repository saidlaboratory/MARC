...
The five implementation faults fixed before any comparison (Section~\ref{sec:entrapment}): the denoiser never
...
Table~\ref{tab:hybrid} is the full mechanism ablation behind Section~\ref{sec:hybrid}: four non-convex
...
Table~\ref{tab:scaling} is the full five-arm battery behind Section~\ref{sec:scaling}; the main text and
...
\section{The Factorization Dichotomy, Tabulated}
\label{app:law}

Table~\ref{tab:law} tabulates the measured dichotomy plotted in Figure~\ref{fig:law} (600 fresh instances
per $n$, Wilson CIs, $K = 8$). Expected restarts $1/q(n)$ are listed over the tested
