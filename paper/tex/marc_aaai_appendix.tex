...
family; 95\% Wilson CIs shown on the random-init arm). Values are those of the committed
...
\toprule
Family & refine cold & refine+Langevin & random-init+polish & multi-start LM & learned hybrid & learned $>$ random? \\
\midrule
...
The regime map (main text, Figure~\ref{fig:regime}) places every measured family on the
...
Table~\ref{tab:law}, the geometry abscissa is the fit of Figure~\ref{fig:law}, and the
three crossover-replication families are placed at slopes inverted from their best-of-8
Levenberg--Marquardt arm through Eq.~\eqref{eq:bestofk}. The real-systems suite is plotted
...
\caption{Left: main-text Table~\ref{tab:repair} drawn, the ranker against its
