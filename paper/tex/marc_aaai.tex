...
matched restarts); selecting failures on two independent streams removes that margin
($N{=}367$ trained-$k$, $331$ held-out-$k$ failures, three optimization seeds): the
...
Holm $p{=}1.3{\times}10^{-4}$) --- but it ties the recipe-only control ($0.249$) and the
best fixed construction ($0.259$), and loses to the matched-budget restart control
($0.270$): real signal, and it is the population prior, with the problem graph
contributing nothing. What kept the question open was the probe: one restart per candidate
...
$[0.161, 0.243]$ ($0.169$ held-out-$k$) --- below the restart control, so even
${\sim}50$ restarts of per-instance measurement select worse than the prior, and no
selector at this budget, learned or oracle, can reach the probe. Accept probability is
diffuse ($11\%$ of candidates ever pass a screen): the probe's edge is a cheap portfolio
sweep over augmented systems, not selection information. Distillation corroborates ---
training the ranker on raw probe outcomes, five-fold cheaper labels and $3{,}874$ training
failures, reaches [P3-SLOT] at the reference budget, as the ceiling predicts. The same
control also prices the trap: grading the pick on the screens that chose it reads $0.762$;
on a fresh stream, $0.199$. The verdict is a closed negative with the mechanism identified:
on two-stream-hard chains, constructions help only as a portfolio tried cheaply, and
``which construction'' carries no signal worth learning at the reference budget.
