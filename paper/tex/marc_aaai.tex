...
multi-start at choosing values? Only narrowly, in a predictable regime. It ties or loses to
random restart on trapped low-dimensional families and wins in a crossover window where
dimension collapses random search, but the advantage is gone once variables couple. Since all
...
reachability; on the separable family one measured constant reproduces the entire best-of-8
curve with no free parameters (mean absolute error 0.012). Nor is the favorable regime an
artifact of our synthetic families: classical multi-start solves all eight standard test
systems we encode, and none falls in it. What
classical solvers make only by enumeration is the discrete choice --- which structural
augmentation renders an unsolvable system solvable. There a candidate-conditioned repair
ranker matches the exhaustive-enumeration ceiling at a fraction of the solver calls, beating
random and a stronger per-candidate probe on accuracy and cost (0.997 vs 0.236 balanced
nonlinear accuracy; $p<10^{-70}$; $0.982\pm0.006$ across seeds). A closing geometry study
...
is portfolio breadth, not learnable signal. We delineate where learned proposals improve on
classical search, and where the residual win is one of cost over an exhaustive classical
recourse, not reach beyond it.
\end{abstract}
...
and make this choice only by enumeration, and for the value decision they are very hard to beat. This paper measures, under controlled
...
the discrete structural decision, a candidate-conditioned repair ranker clears its matched
controls on certificate-grade nonlinear menus, matching the enumeration ceiling at a
fraction of the calls (\S\ref{sec:repair}). Fourth, the same discipline turned
...
authoritative, catching false accepts a numeric tolerance admits; on the real systems, whose
roots are irrational, no exact rational exists to snap to and the numeric stage stands alone.
This gate is the only acceptance criterion in every experiment and the only reward source in
training.
...
beside the interventions; (v) before crediting a learnable selection target, run the cheap
diagnostic --- screen each candidate at the target budget on fresh streams and grade the
argmax on a held-out one --- because if that per-instance measurement cannot beat the
matched-budget control, the intervention's advantage is portfolio breadth, not selection a
model could amortize. On our geometry population the difference is not subtle:
single-stream selection put the construction-repair ceiling at $0.63$--$0.74$ against
$0.09$--$0.27$ for matched restarts, and two-stream selection with a common grading stream
collapsed the margin --- most of it the control, graded off the stream that had defined
failure, coming back to its true rate (\S\ref{sec:repair}).
...
$n \ge 3$ on the coupled family (0/4 significant wins). This oracle samples coordinates
independently, so it bounds only product-of-marginals proposals, not a joint sampler in
principle; what it shows is that the population-marginal information is already exhausted on
the coupled family, and our trained denoiser behaves like a marginal sampler there rather than
recovering the instance-conditional joint. We do not claim a
...
coordinates are pinned only jointly through the chain. Both conditions are necessary: a
learned proposal beats classical search only where reachability collapses \emph{and} the
solution is per-variable separable, and given those it wins when the denoiser has the
capacity to fit the marginals. The separable traps have both and learning wins (except the
two-well family, where the small denoiser did not fit the harder marginals and tied); the
coupled bilinear family fails the first and learning ties; geometry has the first, fails the
second, and ties --- collapse is necessary but not sufficient. Appendix Figure~\ref{fig:regime} places
every measured family on the two axes of the law; the win cell contains the families
...
on its own stream solves $0.698$ at $19.5$ restarts, matching the full-grade enumeration
ceiling ($0.692$; the two are within noise). The probe is not a selector, though --- it
spends the whole menu's budget, and its per-candidate accept probability is diffuse ($11\%$
of candidates ever pass a screen), so it works as a cheap portfolio sweep over augmented
systems, not by identifying which construction to add. A per-instance screen held to the
reference budget confirms this: screening every candidate on two fresh streams and grading
the argmax on a held-out stream scores $0.199$ $[0.161, 0.243]$ ($0.169$ held-out-$k$),
below the restart control --- ${\sim}50$ restarts of per-instance measurement select worse
than the prior does. The learned arms landing on the prior are then the expected outcome,
not a training failure: distilling the probe's own labels into the ranker ($3{,}874$
training failures at five-fold cheaper labels) also converges there ($0.256 \pm 0.004$),
tightening around the prior rather than beating it. The gap between the honest screen
($0.199$) and the same screen scored on the streams that chose it ($0.762$) is the
selection-on-noise trap measured directly. The verdict: on two-stream-hard chains,
constructions help only as a portfolio tried cheaply, and ``which construction'' carries no
signal a selector can exploit at the reference budget.
...
across variables and dimension defeats random restart --- a crossover the unified protocol
places by $n=3$ --- and the advantage disappears under coupling; the factorization law
compresses the characterization into one measured quantity and reproduces the separable
family's random-restart curve without a fitted parameter, closing the route from learned
value proposals to a general claim over classical search here. What survives is the substrate
(exact symbolic residuals, energy guidance, an exact checker around any proposer) and the
division of labor it supports: the decision classical solvers make only by enumeration is not
which values to try but which structure to add, and there the certificate-grade repair ranker
clears its controls on the nonlinear menus, matching the enumeration ceiling at a fraction of
the calls, where ``exactly one solvable option'' is a theorem, not a budget-relative claim. The lesson we generalize is about evaluation itself: each of the four
