...
predicts the failure under variable coupling, and holds on eight named real systems. The
decision that has no classical baseline is discrete: which structural augmentation repairs an
unsolvable system. A candidate-conditioned ranker chooses repairs on certificate-grade menus
--- ``exactly one solvable option'' is a computer-algebra theorem, not a budget-relative claim
--- at $0.997$ versus $0.236$ random ($p<10^{-70}$, seed-robust), beating budget-matched
per-candidate probes on accuracy and cost together. Learning earns its keep on the structural
decision; everything else belongs to the solvers.
% TODO(R28): add one closing sentence with the geometry construction-repair number when
% results/p_geo_repair lands.
\end{abstract}
...
defeats random restart, and this advantage disappears under coupling. the factorization-law section
states the characterization as a parameter-free law in the measured single-start reachability
and validates it on synthetic families and a geometric domain; the structural-repair section
relocates the learned component to the discrete structural decision, where a
candidate-conditioned repair ranker clears matched controls on certificate-grade menus.
Alongside these we contribute the substrate itself (factor graphs with an exact CAS and an
exact accept checker in the loop), a pre-registered entrapment result, a partial cross-family
...
\paragraph{Geometry as construction repair.}
% TODO(R28): fill in numbers from results/p_geo_repair/geo_repair.json + PROVENANCE R28 when
% the run lands; until then this stub carries the design only — cite NO pilot numbers.
The relocation thesis has a real-domain test in flight, on the pruned point chains of
the factorization-law section --- exactly where a trained value denoiser tied restart-matched random
search. Each instance the reference pipeline fails on two independent restart streams
receives a menu of candidate auxiliary constructions (branch pins, mirror gauges, cosine
lifts: the discrete branches of the distance-geometry literature); a candidate-conditioned
ranker scores each construction-augmented graph, and the winner receives one augmented solve.
Labels here are measured and budget-relative --- repair means the construction flips the
outcome at the reference budget, not a CAS certificate as in the menus above --- so the
evaluation carries matched-budget restart controls with a restart-scaling curve, fixed- and
all-construction no-learning recipes, a per-construction probe, and an enumeration ceiling.
% TODO(R28): one sentence of headline numbers (ranker vs restart control vs best-fixed) +
% McNemar p-values, trained-k and held-out-k pools, goes here.
...
budget-relative claim.
% TODO(R28): close with the geometry construction-repair sentence when the numbers land.
