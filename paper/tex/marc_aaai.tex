...
system, it sharply curtails our headline claim. MARC encodes the system as a factor
graph, proposes assignments with a graph-neural diffusion denoiser, polishes them on an exact
computer-algebra energy, and certifies with an exact symbolic checker. Does the learned proposal beat random
...
the best-of-8 curve with no free parameters (mean absolute error 0.012). The favorable regime is not a synthetic
artifact: classical multi-start solves all eight standard test systems we encode, none in
it. What
...
ranker matches the exhaustive-enumeration ceiling at far fewer calls, beating random and a
stronger per-candidate probe on accuracy and cost (0.997 vs 0.236 balanced nonlinear
accuracy; $p<10^{-70}$; $0.982\pm0.006$ across seeds). A closing geometry study
...
is portfolio breadth, not learnable signal. We map where learned proposals improve on classical search, and where the win is cost over
enumeration, not reach beyond it.
\end{abstract}
...
multi-start at the same polish and budget; adding it shrank our own headline claim, and the
corrected claim is this paper's subject.

MARC is an orchestrator: a problem is a factor graph, a diffusion denoiser proposes
assignments, an exact CAS supplies residuals and gradients, energy descent polishes, and a
two-stage checker (numeric gate, then exact symbolic verification) is the sole acceptance
criterion --- the division of labor AlphaGeometry demonstrated in geometry
\citep{trinh2024alphageometry}. The learned proposal is graded against deterministic descent,
annealed Langevin, a mean-prior control, and random multi-start at identical polish and budget.
...
reusable form (\S\ref{sec:entrapment}). Alongside the arc we contribute the substrate itself, a pre-registered entrapment result, a
partial cross-family transfer result, and a MATH-benchmark scope measurement showing
autoformalization, not solving, is the binding constraint. We make no claim against
combinatorial solvers such as DIFUSCO; the problem class differs (Related Work). Every rate
carries $N$ and a 95\% Wilson interval or $z$-test, and negatives are reported in full as the
...
result is confirmation on this substrate, not novelty. What we add is the diagnostic, not the $v^n$ basin count: the measured $\log q(n)$ slope,
one constant that reproduces the best-of-8 curve parameter-free (MAE 0.012), and the
two-condition dissection --- collapse \emph{and} separability --- deciding when a learned
...
instances a stochastic pipeline fails, and conditioning on one bad draw admits instances that
succeed under fresh randomness and grades any control on a stream correlated with the failure
criterion --- so every intervention inherits apparent lift. The discipline: (i) admit an
...
of it the control returning to its true rate (\S\ref{sec:repair}).
...
integers in $[-3,3]$ that random restart over $[-5,5]$ hits readily, leaving nothing to
...
($p < 10^{-4}$ over refine+Langevin) and solves nothing on the other two
...
ceiling at $n = 1$. The law's larger-sample measurement (\S\ref{sec:law}) reads random lower at low $n$
(0.902 at $n=1$) and places the crossover earlier, so we report the geometric collapse, not a
single $n^*$.
...
version, and the Jensen gap this opens is bounded by validating the prediction against a
best-of-8 curve self-measured under identical conditions, not assumed negligible
...
(Figure~\ref{fig:law}; measured on 600 fresh instances of the same family, a larger and
separately drawn sample than Table~\ref{tab:scaling}'s $N = 40$ random arm, which runs a few
...
$n \ge 3$ on the coupled family (0/4 significant wins). Sampling coordinates independently,
this oracle bounds only product-of-marginals proposals, not a joint sampler in principle; what
it shows is that population-marginal information is exhausted here and our trained denoiser
behaves like a marginal sampler rather than recovering the instance-conditional joint. We do not claim a
...
coordinates are pinned only jointly. Both conditions are necessary --- a learned proposal
beats classical search only where reachability collapses \emph{and} the solution is
per-variable separable --- and given those it wins when the denoiser can fit the marginals:
the separable traps have both and learning wins (bar the two-well family, where the small
denoiser did not fit the harder marginals and tied); the coupled bilinear family fails the
first and ties; geometry has the first, fails the second, and ties. Appendix Figure~\ref{fig:regime} places
every measured family on the two axes of the law; learning beats both classical arms only
inside the win cell, and there whenever the denoiser fits the marginals.
...
regime; all are low-dimensional and coupled, where the law expects classical search to
suffice --- read from their structure, not a measured $q(n)$ slope (one instance leaves
nothing to fit), so a consistency check, not the falsifiable slope test. The learned
component is not run per system for the same reason: nothing to amortize.
...
other two patterns in turn gives $0.407$ (offset) and $0.450$ (coupled) against
$\sim$$0.23$ random, so the effect is not specific to one holdout choice. A single model
tested across all three patterns at once --- a harder mixed aggregate, not a pool of the
holdout runs --- reads $0.339$ against $0.249$ for random ($N{=}1{,}200$, $p =
7.8\times10^{-7}$, exact paired McNemar). The
...
($N{=}300$). A cheap-probe control bounds the claim from the other side: a short-budget solver call on
...
short probes miss the gold. On linear menus the same probe saturates ($0.99$ at $4.4$ calls) and enumeration is
...
reaches $0.572$ at $+16$ and $0.725$ at $+32$ --- budget-equivalent or better. Majority-vote
labels over three streams and five-fold data together made the target learnable, and the
ranker separates from random ($0.246 \pm 0.016$ versus $0.185 \pm 0.019$; exact McNemar
$35/8$, Holm $p{=}1.3{\times}10^{-4}$) where single-stream labels at less data never did. But
that is not an edge over the controls: the ranker ties the recipe-only model ($0.249$) and
best fixed construction ($0.259$) and loses to the matched-budget restart control ($0.270$)
--- real signal, but the population prior, with the graph adding nothing. The probe looked
like an opening: one restart per candidate on its own stream solves $0.698$ at $19.5$
restarts, matching the enumeration ceiling ($0.692$, within noise). But it is not a selector
--- it spends the whole menu's budget, and accepts are diffuse ($11\%$ of candidates ever
pass a screen), so it is a portfolio sweep, not a choice of construction. A per-instance
screen held to the reference budget confirms this: screening each candidate on two fresh
streams and grading the argmax on a held-out one scores $0.199$ $[0.161, 0.243]$ ($0.169$
held-out-$k$), below the restart control --- ${\sim}50$ restarts of measurement select worse
than the prior. The learned arms landing on the prior is then expected, not a training miss:
distilling the probe's labels ($3{,}874$ failures) also converges there ($0.256 \pm 0.004$).
The gap between that honest screen ($0.199$) and the same screen scored on the streams that
chose it ($0.762$) is the selection-on-noise trap, measured. The verdict: constructions help only as a cheap portfolio, and ``which construction''
...
not high dimension per se. A sharper caveat: the separable families defining the win cell are block-decomposable, so a
classical solver that read off separability and solved each coordinate independently would
scale linearly there too. Our controls (random restart, Levenberg--Marquardt) use joint
starts, so the crossover is demonstrated against joint-start search, not block decomposition. The law carries its own caveats:
...
We ran the control that decides the question, and the outcome is a characterization. The
learned proposal earns its place only where search is independent across variables and
dimension defeats random restart --- a crossover the unified protocol places by $n=3$ ---
and the advantage disappears under coupling; the factorization law compresses this into one
measured quantity and reproduces the separable family's random-restart curve without a fitted
parameter. What survives is the substrate (exact residuals, energy guidance, an exact checker
around any proposer) and the division of labor it supports: the decision classical solvers
make only by enumeration is not which values to try but which structure to add, and there the
repair ranker matches the enumeration ceiling at a fraction of the calls, where ``exactly one
solvable option'' is a theorem. The lesson we generalize is about evaluation: each of the four
contributions is a control that changed the answer, the last on our own pilot. Populations
defined by stochastic failure are artifacts of the draw unless the protocol makes them stable
--- the discipline of \S\ref{sec:entrapment} costs one extra solve per instance, and results
