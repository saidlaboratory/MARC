...
%   3. switch \bibliographystyle{plainnat} to aaai2026.
...
families, so the hybrid recipe is the useful result there (Section~\ref{sec:hybrid}). On
...
chance, and the learned proposal holds where it fails (Section~\ref{sec:scaling}). On coupled
...
comparisons (Section~\ref{sec:coupled}).
...
and validates it on synthetic families and a geometric domain; Section~\ref{sec:repair}
...
Section~\ref{sec:hybrid} families, $[-8,8]^n$ on the dimension-scaling traps, $[-4,4]^n$ on
...
reachability $v$, so the restart cost grows like $v^{-n}$ (Section~\ref{sec:law}). The learned
proposal holds essentially flat where random fails: 0.950 at $n = 2$, 0.975 at $n = 3$, 0.925
at $n = 4$ --- the flat learned ceiling the law of Section~\ref{sec:law} takes as its input.
...
down that collapse is necessary but not sufficient.
...
Section~\ref{sec:law} --- exactly where a trained value denoiser tied restart-matched random
