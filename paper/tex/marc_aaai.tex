...
Table~\ref{tab:scaling}'s $N = 40$ random arm, which runs a few
...
every measured family on the two axes of the law.
...
$p < 10^{-15}$; the control, not the treatment, carries the sampling variance). The same
...
trained-$k$ failures, three optimization seeds) the
...
restart collapses; on coupled systems it ties or loses at every dimension
tested. Even on favorable families it gains nothing at $n = 1$ (random restart
at ceiling) and degrades to 0.250 at $n = 6$, so the useful window is the crossover region,
...
theorem, and on real systems we did not build, one derived construction repairs failures the
