...
loses to random restart at every dimension tested. Learned diffusion proposals amortize
independent search; they help exactly when solutions are per-variable separable and dimension
defeats random restart. We do not claim more, and we report the negative results with the same
confidence intervals as the positive ones.
\end{abstract}
...
defeats random restart, and this advantage disappears under coupling. Alongside this we
...
The crossover in Table~\ref{tab:scaling} could still be an artifact of independence: those
trap families have per-variable-separable solutions, which a model can represent as $n$
marginals. The stress test is a coupled chained-bilinear family, $x_i + x_{i+1} = s_i$ and
$x_i \cdot x_{i+1} = p_i$, whose solution is a joint object, and where random restart does not
collapse with dimension because polish propagates corrections along the chain.
Table~\ref{tab:coupled} reports best-of-8 with 60 test instances per $n$.
...
is withdrawn pending the clean seed protocol. Finally, every problem family in this paper is
synthetic; the MATH coverage measurement (0/48) quantifies how far the current formalizer is
from real problem text, and we have no result on a real domain yet.
...
advantage disappears under coupling. These boundaries close the route from learned value
proposals to a general claim over classical search on this problem class. What survives is the
