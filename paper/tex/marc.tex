...
than implied.\footnote{A discrete-diffusion structure-selection policy (choosing among $K$
candidate graph augmentations and predicting the defining constant) is built and trained in
the same codebase. Its preliminary evaluation numbers are withdrawn: the evaluation seeds
overlapped the validation seeds used for checkpoint selection, and the harness evaluated a
different data source than training. Clean numbers are pending under a protocol with disjoint
seed ranges asserted at load time, and none are cited in this paper.}
% provenance: RESULTS.md R6 (prose); footnote provenance: RESULTS.md R8
