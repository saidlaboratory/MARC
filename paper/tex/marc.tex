...
(0.550 at $n{=}3$, 0.650 at $n{=}4$, against random $0.050$ and $0.000$), and loses that
advantage once variables couple. Because every method shares one polish operator and one
checker, best-of-$K$ random restart is exactly $1-(1-q(n))^K$ in the single-start reachability
$q(n)$, and the measured slope of $\log q(n)$ decides the regime: learning wins precisely when
acceptance basins factorize across variables and dimension is high. One measured constant
reproduces the entire random-restart curve with no free parameters (mean absolute error 0.012).
The decision classical solvers cannot make is discrete: which structural augmentation turns an
