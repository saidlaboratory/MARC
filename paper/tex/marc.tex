...
\begin{abstract}
Diffusion models are increasingly used to propose solutions to constraint and optimization
problems, but the control that matters most for any proposal mechanism is often missing:
random multi-start under the same refinement budget. We run that control. MARC represents a
continuous algebraic constraint system as a factor graph, proposes candidate assignments with
a graph-neural diffusion denoiser, polishes them by descent on an exact computer-algebra
energy, and accepts an assignment only if it passes an exact symbolic checker. Three findings
result. First, a propose-and-polish hybrid beats cold-start annealed Langevin descent on
trapped non-convex families, but random restarts with the same polish match the learned
proposal there; the recipe is the contribution, not the denoiser. Second, the learned proposal
beats random multi-start only in higher dimension: random restart wins at $n \le 2$, collapses
by $n = 3$, and the learned proposal holds (0.550 at $n=3$, 0.650 at $n=4$). Third, this
advantage does not survive coupling: on chained bilinear systems the learned proposal ties or
loses to random restart at every dimension tested. A factorization law unifies these findings.
Because every method shares one polish operator and one acceptance checker, best-of-$K$ random
restart is exactly $1-(1-q(n))^K$ in the single-start reachability $q(n)$, and the measured
slope of $\log q(n)$ decides the regime: the learned proposal beats classical search precisely
when acceptance basins factorize across variables and dimension is high. One measured constant
reproduces the entire random-restart curve with no free parameters (mean absolute error 0.012).
We do not claim more, and we report the negative results with the same confidence intervals as
the positive ones.
\end{abstract}
...
and spurious basins, so a random start rarely places every point in the right basin. The law
therefore flags geometry as learning-favorable: random search collapses here, so an amortized
proposal that learns each point's marginal placement can win. Solve rates with the classical
polish alone are non-saturated (0.56 in-distribution, 0.28 held out), so the domain is hard
rather than trivially solvable. Training the denoiser on geometry chains against the
random-restart control is the law-guided next experiment.
% pending: geometry learned-vs-random result (D512 checkpoint eval in progress) — insert number here
...
% provenance: RESULTS.md R10 + PROVENANCE R20--R24; results/p_repair
\begin{table}[t]
\centering
\caption{Candidate-conditioned repair ranker against a candidate-only control and random
selection, on three generalization tests (Data Version 6, Wilson 95\% CIs). The ranker clears
both controls on every row; transfer to an unseen relation is positive but partial.}
\label{tab:repair}
\begin{tabular}{lccc}
\toprule
Test & ranker & candidate-only & random \\
\midrule
Linear, unseen pattern ($N{=}400$)      & \textbf{0.565} & 0.343 & 0.283 \\
Balanced nonlinear ($N{=}360$)          & \textbf{0.889} & 0.422 & 0.253 \\
Vieta $\to$ unseen relation ($N{=}150$) & \textbf{0.367} & 0.240 & 0.213 \\
\bottomrule
\end{tabular}
\end{table}

Table~\ref{tab:repair} reports the three tests that probe generalization, each under the Data
Version 6 protocol that randomizes insertion support and withholds the gold auxiliary value from
expression-defined candidates. On an unseen linear pattern the ranker reaches $0.565$ against
$0.283$ for random; on balanced nonlinear menus $0.889$ against $0.253$, with an exact paired
McNemar test on the shared test set giving $p = 4.1\times10^{-46}$ (172 full-only correct versus
4 control-only). Transfer to an unseen nonlinear relation is positive but partial ($0.367$), not
relation-level invariance. Three initialization seeds give a nonlinear mean of $0.881$ with
population SD $0.006$.
...
is $K{=}4$ chance. Right: the $K{=}4$ checkpoint evaluated zero-shot at larger menus stays above
random while the blind enumeration it displaces grows from $2.5$ to $8.6$ solver calls per
instance.}
\label{fig:repair}
\end{figure}
...
invoking the matched solver, it spends a single solver call where blind enumeration averages
$2.53$ (linear) to $2.62$ (nonlinear). The $K = 4$ checkpoint transfers to larger menus without
retraining: at $K = 4/8/16$ its accuracy is $0.540/0.347/0.247$ against random
$0.253/0.140/0.020$, while the enumeration it displaces grows to $8.56$ solver calls per instance
(Figure~\ref{fig:repair}, right). Directly training at $K = 16$ reaches only $0.192$, so the
transferred checkpoint is selected and the direct-training negative is kept in the record. The
scope is menu-based repair on synthetic factor graphs, and the nonlinear ``unsolvable
distractor'' label is an empirical multi-restart certificate at a stated budget, not a
...
disjoint-seed Data Version 6 protocol.}
...
confirmed only on the separable positive and the coupled negative; on geometry the law
predicts that learning can help, and that experiment has not been run.
